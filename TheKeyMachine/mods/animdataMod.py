...
if sys.version_info >= (3, 0):
    import TheKeyMachine.mods.utilsMod as utilsMod
else:
    import utilsMod as utilsMod
...
def prepare(mode):
    ...
    # get curves
    if utilsMod.is_graph_editor_or_dope_sheet():
        curves = utilsMod.get_selected_anim_curves()
        plugs = None
    else:
        nodes = utilsMod.get_selected_objects()
        curves, plugs = utilsMod.get_anim_curves_from_objects(nodes)
    
    # get prev and next values, so we can use them to blend while dragging slider
    is_default = bool(mode == 'blend_to_default')
    is_curve_tangent = bool(mode == 'blend_to_curve')
    
    curve_key_values = {}
    time_range = utilsMod.get_time_slider_range()
    ...
    for plug_idx, curve_node in enumerate(curves):
        ...
        if is_default:
            if plugs:
                default_val = utilsMod.get_attribute_default_value(plugs[plug_idx])
            else:
                default_val = utilsMod.get_anim_curve_default_value(curve_fn)
...
def add_tangent_points_to_key_group(key_group, curve_fn, prev_index, next_index, index=None):
    ...
    if index is None:
        ...
        bezier_set = (utilsMod.get_curve_tangents_bezier_points(curve_fn, prev_index, next_index),)
    else:
        key_group.has_two_segments.append(True)
        bezier_set = (utilsMod.get_curve_tangents_bezier_points(curve_fn, prev_index, index),
                      utilsMod.get_curve_tangents_bezier_points(curve_fn, index, next_index))
