...
if sys.version_info >= (3, 0):
    import TheKeyMachine.mods.animlayersMod as animlayersMod
else:
    import animlayers as animlayersMod
...
def get_anim_curves_from_objects(nodes):
    ...
    animlayersMod.cache.reset()  # always reset cache before querying for animation layers!
    has_anim_layers = animlayersMod.has_anim_layers()
    
    if has_anim_layers and animlayersMod.all_layers_locked():
    ...
    def process_plug(attr, plug, in_array=False):
        ...
        if connections:
            ...
            # find curve in animation layer
            elif has_anim_layers and api in animlayersMod.BLEND_NODE_TYPES:
                # filter out attributes not selected in channelbox
                if channelbox_attr:
                    if in_array:
                        attr_name = plug.partialName(useAlias=True)
                ...
                best_layer = animlayersMod.get_best_layer(plug)
                if not best_layer:
                    return None
                ...
                curve_node = animlayersMod.get_anim_curve(plug, best_layer)
                if curve_node:
...
def get_anim_curve_default_value(anim_curve):
    ...
    if plug:
        ...
        for dst_plug in destinations:
            ...
            while not it.isDone():
                if it.currentNode().apiType() in animlayersMod.BLEND_NODE_TYPES:
                    it.next()
                    if not it.isDone():
