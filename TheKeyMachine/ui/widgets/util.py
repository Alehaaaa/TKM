...
import maya.OpenMayaUI as maya_api
...
def DPI(val):
    return maya_api.MQtUtil.dpiScale(val)
...
def get_maya_qt(ptr=None, qt=QtWidgets.QMainWindow):
    if ptr is None:
        ptr = maya_api.MQtUtil.mainWindow()
...
def get_control_widget(name, qt_type=QtWidgets.QWidget):
    ptr = maya_api.MQtUtil.findControl(name)
    if ptr:
...
def sync_choice_group_button(registry, group_id, button, parent=None):
    ...
    nothing re-queries them. widgets.toolbar_menus.build_declared_menu already
...
def make_row_check_control(parent, object_name, radio=False):
    ...
    setting (see ``tools.hotkeys.controller._tool_choice_setting_rows`` and
    ...
    the same treatment ``widgets.toolbar_menus.build_declared_menu``'s
...
def bind_choice_row_state(control, choice_group, choice_value):
    ...
    construction (``get_checked`` -- see ``tools.hotkeys.controller._tool_choice_setting_rows``),
    ...
    ``tools.registry.apply_choice_value`` publishes the setting's new value
    under its choice id via ``runtime.set_control_state`` every time
