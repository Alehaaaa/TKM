...
def _find_mode(mode_key):
    from TheKeyMachine.tools.slider_tangent import MODES
...
def execute(mode, value, session=None):
    ...
    if mode_data is None:
        raise ValueError("Unknown Tangent slider mode: {}".format(mode))
    ...
    try:
        if session.preview:
            ...
            session.ensure_undo_open()
        apply_tangent_type_blend(session, None, TANGENT_TYPES[mode], value / 100.0)
        return session
    finally:
        if standalone:
            session.finish()
