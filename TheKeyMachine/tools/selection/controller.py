...
def _rig_controls(animated_only=False):
    ...
    for root in _selected_roots():
        for node in _descendant_transforms(root):
            is_control = _is_curve_control(node) or cmds.nodeType(node) == "joint"
            if not is_control or (animated_only and not selectionMod.is_node_animated(node)):
                continue
