...
class AnimationOffsetTimelineTint(timeline.TimelineTint):
    ...
    def eventFilter(self, watched, event):
        if watched in self._handles:
            ...
            if event_type == QtCore.QEvent.MouseMove and self._drag_edge is not None:
                self._set_edge_from_parent_x(
                    self._parent_widget.mapFromGlobal(event.globalPos()).x()
                )
                return True
            if (
    ...
    def _handle_rects(self):
        ...
        return tuple(
            QtCore.QRect(int(round(x - size * 0.5)), y, size, size)
            for x in (tint_rect.left(), tint_rect.right())
        )
...
def show_animation_offset_tint(
    timerange,
    color,
    owner=None,
    key=None,
    ...
    return runtime.get_runtime_manager().register_managed_widget(
        widget, key=key, owner=owner
    )
