...
def _tool_data_for_command(command_name):
    if not command_name:
        return {}
    try:
        return registry.get_tool(command_name)
    except Exception:
        return {}
...
def _tool_choice_setting_rows(tool_id, tool_data):
    ...
    registry tool, so it never reached ``trigger``, Search, or the Hotkeys
    editor before. This turns each of a choice's values into its own row
    through ``tools.registry.tool_choice_settings``, the same declared-menu
    walk ``widgets.toolbar_menus.build_declared_menu`` uses to build the dropdown
    itself, so Search and the Hotkeys editor cannot drift out of sync with
    the live menu. Trigger discovery registers these commands; this function
    only references their names via ``registry.choice_setting_command_name``.
    ...
    ``widgets.toolbar_menus.build_declared_menu`` gets from a ``QActionGroup`` for
    ...
    for choice_item in registry.tool_choice_settings(tool_id):
        ...
        for choice in choices:
            ...
            rows.append({
                "command": registry.choice_setting_command_name(choice_id, value),
                ...
                "set_checked": partial(registry.apply_choice_value, setter, value),
            })
    return rows
...
def _append_toolbox_item_rows(section, seen, title_lookup, icon_lookup, trigger_commands, item):
    ...
    if isinstance(item, dict) and item.get("group"):
        group = registry.get_tool_section(item["group"], resolve_items=False)
        if group:
    ...
    if isinstance(item, dict) and item.get("section"):
        child_section = registry.get_tool_section(item["section"], resolve_items=False)
        if child_section:
    ...
    if item.get("type") == "widget":
        ...
        if tool_key and tool_key in registry.get_tool_definitions():
...
def _slider_modes_from_section(section_data):
    ...
    ``toolbar_widgets.build_slider_section`` starts from when it builds the main
...
def _build_slider_hotkey_section(section_id, section_data):
    ...
    section = {
        ...
        "icon": registry.get_section_icon(section_id),
        "commands": [],
    }
...
def _append_background_runner_rows(section, seen, title_lookup, icon_lookup, trigger_commands):
    ...
    for runner_id, spec in background_runners.get_runner_specs().items():
        ...
        # (background_runners.RUNNER_COMMAND_IDS) -- one table, not a second
        ...
        # already applies -- background_runners.py's specs carry only
        ...
        tool = registry.get_tool(command_name) if trigger.has_command(command_name) else {}
        tooltip = tool.get("tooltip")
        # background_runners.py's specs only ever carry a plain "label" (no
        ...
        _append_section_row(
            ...
            trigger_commands,
            {
                ...
                "set_checked": lambda enabled, rid=runner_id: background_runners.set_runner_enabled(rid, enabled),
...
def _iter_hotkey_tool_sections():
    ...
    # English text.
    for section_id, raw_section_data in registry.get_section_definitions().items():
        if raw_section_data.get("hotkey_only") and raw_section_data.get("hotkeys") is not False:
            seen.add(section_id)
            section_data = registry.get_tool_section(section_id, resolve_items=False) or raw_section_data
            yield section_id, section_data

    for layout_id in ("main", "graph"):
        for section_data in registry.get_toolbar_sections(layout_id, resolve_items=False):
...
class HotkeyStatusResolver(object):
    ...
    def _external_assignment(self, combo, ckey):
        # These queries are scoped to Maya's active hotkey set and therefore
        # are the authoritative source for the currently bound command.
...
class TriggerHotkeysDialog(cd.QFlatDialog):
    def __init__(self, parent=None):
        ...
        header_text = registry.get_tool("hotkeys_window").get("label") or "Hotkeys"
    ...
    def _begin_catalog_load(self):
        ...
        registry -- building it synchronously here, like the dialog used to,
    ...
    def _build_content(self, parent):
        ...
        hotkeys_label = registry.get_tool("hotkeys_window").get("label") or "Hotkeys"
